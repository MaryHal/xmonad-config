...
data DzenConf = DzenConf 
    ...
    , barwidth      :: Maybe DzenWidth -- ^ width
    , barheight     :: Maybe Int       -- ^ line height
    , alignment  :: Maybe TextAlign -- ^ alignment of title window
    , barfont    :: Maybe String    -- ^ font
...
dzenArgs d = do
    ...
    w <- mkWidth (screen d) (barwidth d)

    let s = fmap (+1) $ screen d -- the -xs arg is 1 index

    return $ addOpt ("-fn", fmap quote $ barfont    d)
          ...
          ++ addOpt ("-h" , fmap show  $ barheight  d)
...
defaultDzenXft = defaultDzen { barfont = Just "Verdana-8" }
